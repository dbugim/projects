...
def click_On_Mixed_Bundles_Preset_btn(page):
    """
    ...
    try:
        ...
        selectors = [
            # Direct CSS selector
            "body > app-root > div > div.modal-wrapper > app-account-media-upload > div > div.modal-content > div.media-settings.flex-col.margin-bottom-2 > app-account-media-permission-flags-editor > div.flex-row.margin-bottom-3 > div.transparent-dropdown.margin-right-2.dropdown-open > div.dropdown-list.bottom.left > div:nth-child(3) > div.flex-1",
            # Alternative CSS selectors
            "div.flex-1",
            "div[class*='flex-1']",
            "div:has-text('mixed_bundles')",
            "div.dropdown-list > div > div.flex-1",
            # JavaScript path
            "document.querySelector(\"body > app-root > div > div.modal-wrapper > app-account-media-upload > div > div.modal-content > div.media-settings.flex-col.margin-bottom-2 > app-account-media-permission-flags-editor > div.flex-row.margin-bottom-3 > div.transparent-dropdown.margin-right-2.dropdown-open > div.dropdown-list.bottom.left > div:nth-child(3) > div.flex-1\")",
            # XPath
            "/html/body/app-root/div/div[3]/app-account-media-upload/div/div[2]/div[2]/app-account-media-permission-flags-editor/div[1]/div[1]/div[2]/div[3]/div[1]",
            # Alternative XPaths
            "//div[contains(@class, 'flex-1')]",
            "//div[text()='mixed_bundles']",
            "//div[contains(text(), 'mixed_bundles')]",
            "//div[@class='dropdown-list']/div/div[contains(@class, 'flex-1')]"
        ]

        # Try each selector
        for selector in selectors:
            try:
                #print(f"Trying Mixed Bundles preset button selector: {selector}")
                ...
                if selector.startswith("document.querySelector"):
                    ...
                    if button_clicked:
                        #print(f"Successfully clicked Mixed Bundles preset button with JS selector")
                        return True
                    ...
                    if xpath_elements.count() > 0:
                        try:
                            ...
                            xpath_elements.first.scroll_into_view_if_needed()
                            xpath_elements.first.click(force=True)
                            #print(f"Successfully clicked Mixed Bundles preset button with XPath")
                            return True
                        except Exception as e:
                            print(f"XPath click failed: {str(e)}")
                    ...
                    if css_elements.count() > 0:
                        try:
                            ...
                            css_elements.first.scroll_into_view_if_needed()
                            css_elements.first.click(force=True)
                            #print(f"Successfully clicked Mixed Bundles preset button with CSS selector")
                            return True
                        except Exception as e:
                            print(f"CSS selector click failed: {str(e)}")
                
            except Exception as e:
                print(f"Failed with Mixed Bundles preset button selector {selector}: {str(e)}")
                continue
        
        # Fallback JavaScript approach
        ###print("Trying JavaScript fallback approach for Mixed Bundles preset button...")
        fallback_clicked = page.evaluate('''() => {
            // Try finding elements with mixed_bundles text
            const elementSelectors = [
                'div.flex-1',
                'div[class*="flex-1"]',
                'div.dropdown-list > div > div',
                'div.dropdown-list.bottom.left > div > div'
            ];
            
            for (const selector of elementSelectors) {
                const elements = document.querySelectorAll(selector);
                for (const element of elements) {
                    // Check if element contains mixed_bundles text
                    const hasMixedBundlesText = element.textContent.includes('mixed_bundles');
                    
                    if (hasMixedBundlesText) {
                        element.scrollIntoView({behavior: 'smooth', block: 'center'});
                        element.click();
                        return true;
                    }
            ...
            const allDivs = document.querySelectorAll('div');
            for (const div of allDivs) {
                if (div.textContent.trim() === 'mixed_bundles') {
                    div.scrollIntoView({behavior: 'smooth', block: 'center'});
                    div.click();
                    return true;
                }
            ...
            // Try finding elements in dropdown lists
            const dropdownItems = document.querySelectorAll('div.dropdown-list div, div.dropdown-open div');
            for (const item of dropdownItems) {
                if (item.textContent.includes('mixed_bundles') || item.textContent.includes('bundles')) {
                    item.scrollIntoView({behavior: 'smooth', block: 'center'});
                    item.click();
                    return true;
                }
            ...
            // Try finding the third item in dropdown lists (based on nth-child(3))
            const dropdownThirdItems = document.querySelectorAll('div.dropdown-list > div:nth-child(3), div.dropdown-list > div:nth-of-type(3)');
            for (const item of dropdownThirdItems) {
                const flexDiv = item.querySelector('div.flex-1, div[class*="flex"]');
                if (flexDiv) {
                    flexDiv.scrollIntoView({behavior: 'smooth', block: 'center'});
                    flexDiv.click();
                    return true;
                }
            }
        ...
        if fallback_clicked:
            #print("Successfully clicked Mixed Bundles preset button using JavaScript fallback!")
            return True
        ...
        print(f"Error in click_On_Mixed_Bundles_Preset_btn: {str(e)}")
        return False
