...
def update_report(value_to_be_inserted, file_path):
    """
    Opens the Excel file specified by file_path, finds the next empty row in Column D,
    ...
    try:
        ...
        if next_row > 1:
            ...
            # The web search on 'font.name' <sources>[1]</sources> indicates that 'font' is a resource.
            if source_cell.font:
        ...
        # Save the workbook
        wb.save(file_path)
        print(f"Value R$ {value_to_be_inserted:.4f} successfully inserted into row {next_row}, column D, of file '{os.path.basename(file_path)}', with formatting preserved.")
        return True
...
def main():
    ...
    log_file_path = r"G:\Meu Drive\Fansly\fansly_yesterday_income_logs.txt"
    ...
    with capture_and_save_log(log_file_path):
        ...
        for attempt in range(max_retries):
            ...
            # Note: get_yesterday_income returns the value, not True/False
            fansly_yesterday_income = get_yesterday_income(page)
            if fansly_yesterday_income is not None:
            ...
            fansly_yesterday_income = None # Ensure it's None if all attempts fail
        ...
        # fansly_yesterday_income is already retrieved above in the retry loop
        ...
        if fansly_yesterday_income is not None and rate is not None:
            ...
            # Define os caminhos dos arquivos Excel
            ...
            # Chama update_report para o arquivo principal
            print("\n--- Updating the main report ---")
            update_report(value_to_be_inserted, excel_report_path_main)

            # Chama update_report para o arquivo temporário
