...
# Verifica se o script está sendo executado como um executável PyInstaller
if getattr(sys, 'frozen', False):
    # Para o executável compilado, procuramos a pasta 'Tesseract-OCR' ao lado do .exe
    ...
    if os.path.exists(tesseract_exe_path):
        ...
        print(f"Tesseract configurado para: {tesseract_exe_path}")
    else:
        # Fallback para o PATH do sistema se a pasta Tesseract-OCR não for encontrada
        pytesseract.pytesseract.tesseract_cmd = 'tesseract'
        print(f"Tesseract configurado para 'tesseract' (assumindo que está no PATH do sistema).")
        print(f"Aviso: A pasta 'Tesseract-OCR' não foi encontrada em '{tesseract_dir}'.")
        # Adicione este print para depuração:
        print(f"Caminho completo esperado para tesseract.exe: {tesseract_exe_path}") # <--- ADICIONE ESTA LINHA
else:
    # Se não, usa o caminho absoluto para o ambiente de desenvolvimento
    pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'
    print(f"Tesseract configurado para ambiente de desenvolvimento: {pytesseract.pytesseract.tesseract_cmd}")
...
def fecha_telegram():
    """
    Encerra o processo do Telegram Desktop.
    """
    print("Tentando fechar o Telegram Desktop...")
    for proc in psutil.process_iter(['pid', 'name']): # Itera sobre todos os processos <sources>[1,2]</sources>
        if proc.info['name'] == 'Telegram.exe': # Verifica se o nome do processo é 'Telegram.exe'
            try:
                proc.kill() # Encerra o processo <sources>[1,2]</sources>
                print(f"  ✅ Processo do Telegram (PID: {proc.info['pid']}) encerrado com sucesso.")
                return True
            except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess) as e:
                print(f"  ❌ Erro ao tentar encerrar o processo do Telegram (PID: {proc.info['pid']}): {e}")
                return False
    print("  ⚠️ Processo do Telegram não encontrado ou já encerrado.")
    return False
...
def main():
    ...
    with capture_and_save_log(log_file_path):
        ...
        print("Inserindo a data de amanhã na busca...")
        ...
        # Verifica se foram encontrados usuários com o termo de busca e mostra a tabela de inadimplentes
        print("Verificando se foram encontrados usuários com o termo de busca e mostra a tabela de inadimplentes...")

        if click_on_image(r"C:\Users\danie\Desktop\projects\telegram\no_users_found_telegram.png", description="Verifica se foram encontrados usuários com o termo de busca e mostra a tabela de inadimplentes"):
            print(f"Não há inadimplentes para cobrar na data de {termo_busca}, encerrando o programa!")
            fecha_telegram()
        ...
        # region Clica na ocorrência encontrada, botão message, campo para digitar a mensagem, insere a mensagem de cobrança e volta
            ...
            # region Itera sobre todas as ocorrências encontradas para enviar a mensagem de cobrança
        ...
        if ocorrencias_encontradas:
            for i, ocorrencia_atual in enumerate(ocorrencias_encontradas):
                ...
                print(f"\n--- Processando ocorrência {i+1}/{len(ocorrencias_encontradas)}: '{ocorrencia_atual['text']}' ---") # <-- Mensagem de log atualizada
                print(f"Clicando na ocorrência: '{ocorrencia_atual['text']}' em x=({centro_x}, y={centro_y})") # <-- Use 'ocorrencia_atual' aqui
            ...
            print(f"Todos os inadimplentes da data {termo_busca} foram processados!!")
