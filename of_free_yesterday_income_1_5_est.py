...
def update_report(value_to_be_inserted, file_paths):
    """
    Opens the Excel files specified by file_paths, finds the next empty row in Column E,
    ...
    for file_path in file_paths:
        try:
            ...
            while ws.cell(row=next_row, column=column_e).value is not None:
                next_row += 1

            # Get the cell where the value will be inserted
            target_cell = ws.cell(row=next_row, column=column_e)

            # Apply formatting from the cell above, if available
            if next_row > 1:
                source_cell = ws.cell(row=next_row - 1, column=column_e)
            ...
            # Save the workbook
            wb.save(file_path)
            print(f"Value R$ {value_to_be_inserted:.4f} successfully inserted into row {next_row}, column E, of file '{os.path.basename(file_path)}', with formatting preserved.")
...
def main():
    ...
    log_file_path = r"G:\Meu Drive\Financeiro\of_free_yesterday_income_logs.txt"
    ...
    with capture_and_save_log(log_file_path):
        ...
        try:
            ...
            # Agora, use yesterday_earnings_value para o cálculo
